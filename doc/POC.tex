...
Three services (full source in \texttt{docker-compose.yml}): \texttt{app},
plus \texttt{db}/\texttt{db-init} standing in for RDS locally --
production points \texttt{DATABASE\_URL} at real RDS instead. \texttt{app}
hardening: read-only rootfs (tmpfs for upload spooling),
\texttt{cap\_drop: ALL}, \texttt{no-new-privileges}, explicit non-root UID.
Host port 80 $\to$ container 8000.

\begin{tipbox}
\texttt{db} can't go distroless (no such SQL Server image exists) but runs
non-root by default. \texttt{cap\_drop: ALL} alone breaks it: \texttt{sqlservr}
has \texttt{cap\_net\_bind\_service} as a \emph{file} capability, which the
kernel won't grant at exec unless it's in the caller's bounding set --
fixed by adding back just \texttt{NET\_BIND\_SERVICE}.
\end{tipbox}

\subsection{Environment Templates}

\texttt{.env.example} is for running the app directly; \texttt{.env.docker.example}
is what \texttt{docker compose up} reads for \texttt{\$\{VAR\}} substitution
(not mounted into the container; \texttt{DATABASE\_URL} is assembled from
\texttt{MSSQL\_*} + the \texttt{db} service name). Both templates are in
the repository root.
...
# Edit .env: set AWS_REGION and the three S3_BUCKET_* names for this region.
# Leave AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY blank -- the IAM instance
# role supplies credentials automatically. Set MSSQL_SA_PASSWORD to a
# strong value if running the bundled db container; if pointing at real
# RDS instead, skip the db/db-init services (docker compose up app) and
# set DATABASE_URL directly as an override.

docker compose up -d --build
docker compose ps          # app, db, db-init all healthy/exited-0
...
\subsection{Pointing at Real RDS Instead of the Bundled \texttt{db} Container}
Once RDS exists, set \texttt{DATABASE\_URL} in \texttt{.env} and bring up
only \texttt{app} with \texttt{--no-deps} (otherwise Compose still starts
\texttt{db}/\texttt{db-init} first, per their \texttt{depends\_on}):
\begin{lstlisting}[style=bashStyle]
echo 'DATABASE_URL=mssql+pymssql://admin:devpassword@file-backup-db.c9g4u6ekudby.ap-southeast-1.rds.amazonaws.com:1433/filebackup' >> .env
