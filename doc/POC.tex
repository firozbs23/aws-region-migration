...
practical Ubuntu EC2 instance, port 8000 forwarded to the container.
...
\begin{tikzpicture}[
  ...
  \draw[arr] (client) -- node[above, font=\scriptsize] {HTTPS :8000} (app);
...
\begin{itemize}
  \item \textbf{EC2} runs one thing: the Docker container (Section~\ref{sec:docker}), host port 8000 $\to$ container 8000. Stateless -- no file data touches the container's own (read-only) filesystem, so migrating compute is just a new instance running the same image.
...
port 22 from admin IP only, port 8000 from whatever needs Swagger/API access.
...
\texttt{db\_host}, \texttt{db\_password}, \texttt{db\_name}; method \texttt{bucket\_for\_category(category)} maps
...
The database is AWS RDS SQL Server -- set \texttt{DB\_PASSWORD} (and
\texttt{DB\_HOST} on migration) in \texttt{.env}. \texttt{app} hardening: read-only rootfs
(tmpfs for upload spooling), \texttt{cap\_drop: ALL},
\texttt{no-new-privileges}, explicit non-root UID. Host port 8000 $\to$
container 8000.

\subsection{Environment Templates}

\texttt{.env.example} is the template for both direct app runs and
\texttt{docker compose up}. Copy to \texttt{.env} and set \texttt{DB\_PASSWORD}
to your RDS master password. Compose reads \texttt{.env} for
...
Full request cycle through the hardened container & upload $\to$ correct bucket per category $\to$ category-filtered list $\to$ download (byte-for-byte match) $\to$ delete $\to$ 404, all through host port 8000, with \texttt{read\_only: true} and \texttt{cap\_drop: ALL} in effect \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}
  ...
  \item \textbf{Security groups}: \texttt{dev-ssh-access-sg} (inbound 22 from admin IP), \texttt{dev-web-access-sg} (inbound 8000 from API/Swagger consumers), \texttt{dev-mssql-access} (inbound 1433 from \texttt{dev-web-access-sg} only). EC2 gets both \texttt{dev-ssh-access-sg} and \texttt{dev-web-access-sg}.
...
sqlcmd -S file-backup-db.c9g4u6ekudby.ap-southeast-1.rds.amazonaws.com -U admin -P 'demo-password' -C -N \
...
cp .env.example .env
# Edit .env: set DB_PASSWORD to your RDS master password, DB_HOST, AWS_REGION,
# and the three S3_BUCKET_* names for this region. Leave AWS_ACCESS_KEY_ID /
...
\begin{tipbox}
Port 8000 $\to$ container 8000, Swagger reachable with no reverse-proxy step.
Before real usage: TLS termination (Nginx/ALB) in front, \texttt{dev-web-access-sg}
restricted to 443 (or keep 8000 if no TLS layer yet).
\end{tipbox}
...
\texttt{DB\_HOST}/\texttt{DB\_PASSWORD}, and the EC2 instance itself (new instance, same
...
DB_HOST=file-backup-db-sydney.xxxxx.ap-southeast-2.rds.amazonaws.com
...
\begin{lstlisting}[style=bashStyle]
docker compose up -d --build
curl http://<new-ec2-ip>:8000/api/v1/region-info
\end{lstlisting}
\end{enumerate}
